...
## TMCP v2.4 Quality Gates

This repository now ships the **TMCP v2.4** risk workflow. It aggregates scanner outputs, computes the dynamic Process/Architecture (P/A) signal, and publishes an emoji-rich summary directly on every pull request via `.github/workflows/tmcp_workflow_v2_4.yml`.

### Running TMCP locally

```shell
# Aggregate SARIF/JSON files into tmcp_summary.json
python tmcp_aggregate.py --sources tmcp_scans --output tmcp_summary.json --weights-output weights_used.json

# Compute dynamic P/A + R* values (uses GitHub event + coverage summary)
python tmcp_compute_pa.py --event tests/fixtures/tmcp/event.json --summary tmcp_summary.json --coverage coverage/coverage-summary.json --output pa.json

# Produce the same emoji summary comment used in CI
python scripts/tmcp_pr_summary.py --pa pa.json --summary tmcp_summary.json --out tmcp_comment.md
```

The workflow automatically runs `bandit`, `pip-audit`, and `detect-secrets`, executes the dedicated `tests/test_tmcp.py` suite with coverage, and uploads the resulting `tmcp_summary.json`, `weights_used.json`, and `pa.json` artifacts for auditing.

### Scenario calculators and visuals

All supporting assets generated by `scripts/generate_tmcp_assets.py` live in `docs/tmcp/`:

- `TMCP_Scenario_Calculator.xlsx`: interactive what-if planner with gates and scenario charting
- `TMCP_Scorecard_v2.2.xlsx`: detailed scorecard showing Process/Architecture contributors, gates, and verdict
- `tmcp_formula.png` and `tmcp_steps.png`: quick-reference diagrams for the TMCP formula and execution flow

Use `python scripts/generate_tmcp_assets.py` whenever you need to recreate the workbooks or diagrams.
