...
# MAGIC
# MAGIC Υποσημείωση1: Τα ΑΤ_Institutes που περιέχουν χώρες από τις λίστες , κατηγοριοποιήθηκαν ως Διαβατήρια (lines 303-305 of executable). Οι διαφορές με το προηγούμενο run είναι:
# MAGIC +9079 καινούρια Διαβατήρια που είχαν κατηγοριοποιηθεί ως ταυτότητες 
# MAGIC ΑT_Info         AT_info_Wemetrix_old  count         AT_info_Wemetrix_new
# MAGIC Unknown_Format	AT_Format	150777	    143639	      +7138	Passport_Format
# MAGIC AT_Format	      AT_Format	11197514	  11195573	    +1941	Passport_Format
# MAGIC
# MAGIC
# MAGIC
...
df_master_table = spark.sql(f"""SELECT a.*, b.AT_INSTITUTE, b.ADT_INSTITUTE, b.VALID_DATE_FROM_TAUTOTITA ,b.VALID_DATE_TO_TAUTOTITA ,
                            b.CA_Application_Date ,b.VALID_DATE_FROM_DIAVATIRIO ,b.VALID_DATE_TO_DIAVATIRIO ,b.Determination_ID 
                            from {input_table} a left join wemetrix.mnemosyne_AT b  on a.contract_account_id = b.contract_account_id   """)
...
df_results = (df_master_table1\
    ...
    .withColumn("AT_Info_Wemetrix",\
        F.when(F.col("AT").rlike("|".join(at_passport_institutes)),F.lit("Passport_Format"))\
            .otherwise(F.col("AT_Info_Wemetrix")))\
    .withColumn("AT_Info_Wemetrix",\
        F.when( (F.col("AT_Info_Wemetrix")=="AT_Format") & ( (F.col("AT_INSTITUTE").isin(countries_gr))  | (F.col("AT_INSTITUTE").isin(countries_en)) ), F.lit("Passport_Format")) \
            .otherwise(F.col("AT_Info_Wemetrix")) )
...
df_results1 = df_results.withColumn(
    "ClAT_IIS_Wemetrix",
    when((F.trim(col("AT_Info_Wemetrix"))!= "Army_Police_Format") & (F.trim(col("AT_Info_Wemetrix"))!= "Passport_Format") & (F.trim(col("AT_Info_Wemetrix"))!= "AT_Format") ,F.trim(col("ClAT_IIS")))
    .when((F.trim(col("AT_Info_Wemetrix")) == "Army_Police_Format") ,F.concat(F.lit("ΣΤΡ_"),F.regexp_extract(F.col("ClAT"), '\d+', 0)))
    .when((F.trim(col("AT_Info_Wemetrix")) == "AT_Format") ,F.coalesce(F.trim(col("ClAT_IIS")),F.trim(col("ClAT")) ))
).withColumn(
    "ClAT_IIS_Wemetrix_passport",
    when((F.trim(col("AT_Info_Wemetrix")) == "Passport_Format") ,F.regexp_replace(F.col("ClAT"),r'(\D+)$',""))
).withColumn(
    ...
    when( (F.trim(col("Passport_Info_Wemetrix"))!= "Passport_Format") & (F.trim(col("Passport_Info_Wemetrix"))!= "AT_Format") ,F.trim(col("ClPassport_IIS"))) 
    .when((F.trim(col("Passport_Info_Wemetrix")) == "Passport_Format") ,F.regexp_replace(F.col("ClPassport"),r'(\D+)$',""))
).withColumn(
    "ClPassport_IIS_Wemetrix_AT",
    when((F.trim(col("Passport_Info_Wemetrix")) == "AT_Format") ,F.regexp_replace(F.col("ClPassport"),r'(\D+)$',""))
).withColumn(
...
df_results1 = df_results1.withColumn(
    "ClAT_IIS_Wemetrix",
    when(F.trim(col("ClAT_IIS_Wemetrix")).isNull() ,F.trim(col("ClPassport_IIS_Wemetrix_AT")) )
    .otherwise(F.trim(col("ClAT_IIS_Wemetrix")))
).withColumn(
    "ClPassport_IIS_Wemetrix",
    when((F.trim(col("ClPassport_IIS_Wemetrix")).isNull()) ,F.trim(col("ClAT_IIS_Wemetrix_passport")))
    .otherwise(F.trim(col("ClPassport_IIS_Wemetrix")))
)

df_results1 = df_results1.withColumn(
    "AT_Info_Wemetrix",
    when( F.trim(col("ClAT_IIS_Wemetrix"))== F.trim(col("ClPassport_IIS_Wemetrix_AT")),F.lit("AT_Format"))
    .otherwise(F.trim(col("AT_Info_Wemetrix")))
).withColumn(
    "Passport_Info_Wemetrix",
    when( F.trim(col("ClPassport_IIS_Wemetrix"))== F.trim(col("ClAT_IIS_Wemetrix_passport"))  ,F.lit("Passport_Format"))
    .otherwise(F.trim(col("Passport_Info_Wemetrix")))
)
