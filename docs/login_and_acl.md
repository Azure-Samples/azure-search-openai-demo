...
- `AZURE_DISABLE_APP_SERVICES_AUTHENTICATION`: Disables [use of the integrated authentication for App Services](https://learn.microsoft.com/azure/app-service/overview-authentication-authorization). Built-in authentication to the chat app is used instead. Useful when you want to provide programmatic access to the chat endpoints with authentication.
