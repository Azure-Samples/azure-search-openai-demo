...
Provides both local (pypdf) and cloud (Azure Document Intelligence) parsers.
"""
...
from ..models import ImageOnPage, Page
...
class LocalPdfParser(Parser):
    ...
    async def parse(self, content: IO) -> AsyncGenerator[Page, None]:
        """Parse PDF using local pypdf library.

        Args:
            content: File-like object containing PDF data

        Yields:
            Page objects with extracted text
        """
...
class DocumentAnalysisParser(Parser):
    """
    Concrete parser backed by Azure AI Document Intelligence that can parse
    many document formats into pages.
    ...
    def __init__(
        ...
        """Initialize the Document Intelligence parser.

        Args:
            endpoint: Azure Document Intelligence endpoint URL
            credential: Authentication credential
            model_id: Document Intelligence model to use
            process_figures: Whether to extract and process figures
        """
    ...
    async def parse(self, content: IO) -> AsyncGenerator[Page, None]:
        ...
        Args:
            content: File-like object containing document data

        Yields:
            Page objects with extracted text and figures
        """
        ...
        async with DocumentIntelligenceClient(
            ...
            # Always convert to bytes up front
            try:
                content.seek(0)
            except Exception:
                pass
    ...
    async def figure_to_image(doc: pymupdf.Document, figure: DocumentFigure) -> ImageOnPage:
        """Convert a Document Intelligence figure to an ImageOnPage."""
    ...
    def table_to_html(table: DocumentTable) -> str:
        """Convert a Document Intelligence table to HTML."""
