"""
Media describers for generating image descriptions.
"""
...
class ContentUnderstandingDescriber(MediaDescriber):
    """Media describer using Azure Content Understanding service."""
    ...
    def __init__(self, endpoint: str, credential: AsyncTokenCredential):
        """Initialize the Content Understanding describer.
    ...
    async def poll_api(self, session, poll_url, headers):
        """Poll the API until the operation completes."""
    ...
    async def create_analyzer(self):
        """Create the Content Understanding analyzer if it doesn't exist."""
    ...
    async def describe_image(self, image_bytes: bytes) -> str:
        """Generate a description using Content Understanding."""
        async with aiohttp.ClientSession() as session:
...
class MultimodalModelDescriber(MediaDescriber):
    """Media describer using OpenAI multimodal models (GPT-4 Vision)."""

    def __init__(self, openai_client: AsyncOpenAI, model: str, deployment: Optional[str] = None):
        """Initialize the multimodal model describer.
    ...
    async def describe_image(self, image_bytes: bytes) -> str:
        ...
        def before_retry_sleep(retry_state):  # noqa: ARG001
