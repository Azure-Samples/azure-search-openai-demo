...
from ingestion.parsers import FileProcessor
from ingestion.models import File, ImageOnPage, Page
from ingestion.uploader import build_file_processors
from ingestion.config import OpenAIHost
from ingestion.strategies.file import process_text
...
def select_processor_for_filename(filename: str, file_processors: dict[str, FileProcessor]) -> FileProcessor:
    """Select the appropriate file processor based on file extension."""
    ext = os.path.splitext(filename)[1].lower()
    processor = file_processors.get(ext)
    if processor is None:
        # Default to .txt processor if extension not found
