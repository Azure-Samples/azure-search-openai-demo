...
class Section:
    ...
    def __init__(
        self, split_page: SplitPage, content: File, category: Optional[str] = None
    ):
...
class SearchManager:
    ...
    def __init__(
        ...
        self.embedding_dimensions = (
            self.embeddings.open_ai_dimensions if self.embeddings else None
        )
    ...
    async def create_index(self):
        logger.info(
            "Checking whether search index %s exists...", self.search_info.index_name
        )
        ...
        async with self.search_info.create_search_index_client() as search_index_client:
            ...
            if self.search_info.index_name not in [
                ...
                if self.use_int_vectorization:
                    logger.info(
                        "Including parent_id field for integrated vectorization support in new index"
                    )
                    fields.append(
                        SearchableField(
                ...
                if embedding_field:
                    ...
                    if (
                        ...
                        raise ValueError(
                            "Text vector search profile, algorithm and compression must be set"
                        )
                ...
                if image_embedding_field:
                    logger.info(
                        "Including %s field for image vectors in new index",
                        image_embedding_field.name,
                    )
                    fields.append(image_embedding_field)
                    if (
                        image_vector_search_profile is None
                        or image_vector_algorithm is None
                    ):
                        raise ValueError(
                            "Image search profile and algorithm must be set"
                        )
                ...
                logger.info(
                    "Search index %s already exists", self.search_info.index_name
                )
                existing_index = await search_index_client.get_index(
                    self.search_info.index_name
                )
                if not any(
                    field.name == "storageUrl" for field in existing_index.fields
                ):
                    logger.info(
                ...
                if embedding_field and not any(
                    field.name == self.field_name_embedding
                    for field in existing_index.fields
                ):
                    logger.info(
                        "Adding %s field for text embeddings", self.field_name_embedding
                    )
                    existing_index.fields.append(embedding_field)
                    if existing_index.vector_search is None:
                        raise ValueError(
                            "Vector search is not enabled for the existing index"
                        )
                    if text_vectorizer is not None:
                ...
                if image_embedding_field and not any(
                    ...
                    if (
                        ...
                        raise ValueError(
                            "Image vector search profile and algorithm must be set"
                        )
                    if existing_index.vector_search is None:
                        raise ValueError(
                            "Image vector search is not enabled for the existing index"
                        )
                    if existing_index.vector_search.profiles is None:
                        existing_index.vector_search.profiles = []
                    existing_index.vector_search.profiles.append(
                ...
                if existing_index.semantic_search:
                    if not existing_index.semantic_search.default_configuration_name:
                        ...
                        existing_index.semantic_search.default_configuration_name = (
                            "default"
                        )

                    if existing_index.semantic_search.configurations:
                        existing_semantic_config = (
                            existing_index.semantic_search.configurations[0]
                        )
                        if (
                            existing_semantic_config.prioritized_fields
                            and existing_semantic_config.prioritized_fields.title_field
                            and not existing_semantic_config.prioritized_fields.title_field.field_name
                            == "sourcepage"
                        ):
                            logger.info(
                            ...
                            existing_semantic_config.prioritized_fields.title_field = (
                                SemanticField(field_name="sourcepage")
                            )
                ...
                if existing_index.vector_search is not None and (
                    ...
                    if self.embeddings is not None and isinstance(
                        self.embeddings, AzureOpenAIEmbeddingService
                    ):
                        logger.info(
    ...
    async def create_agent(self):
        if self.search_info.agent_name:
            ...
            async with (
                self.search_info.create_search_index_client() as search_index_client
            ):
            ...
            for batch_index, batch in enumerate(section_batches):
                ...
                if image_embeddings:
                    for i, (document, section) in enumerate(zip(documents, batch)):
                        document["imageEmbedding"] = image_embeddings[
                            section.split_page.page_num
                        ]
