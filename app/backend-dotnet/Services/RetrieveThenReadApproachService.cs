...
{
    public class RetrieveThenReadApproachService : IApproachService
    {
        ...
        public RetrieveThenReadApproachService(SearchClient searchClient, IKernel kernel)
        {
            ...
            var promptTemplate = new PromptTemplate(this.TEMPLATE, promptConfig, this.kernel);
            var functionConfig = new SemanticFunctionConfig(promptConfig, promptTemplate);
            this.function = this.kernel.RegisterSemanticFunction("RetrieveThenRead", functionConfig);
        }
        public async Task<IApproachService.Reply> RunAsync(ContextVariables variable, PromptTemplateConfig config)
        {
            ...
            var documents = await this.searchClient.SearchAsync<SearchDocument>(question, searchOption);
            if(documents.Value != null)
            {
                ...
                var answer = await this.kernel.RunAsync(variable, this.function);
