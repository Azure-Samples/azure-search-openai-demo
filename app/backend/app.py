...
# Uses the current user identity to authenticate with Azure OpenAI, Cognitive Search and Blob Storage (no secrets needed, 
# just use 'az login' locally, and managed identity when deployed on Azure)
azure_default_credential = DefaultAzureCredential(exclude_shared_token_cache_credential = True)

if AZURE_SEARCH_API_KEY is not None:
    # Use the API keys to authenticate with Cognitive Search
    azure_search_credential = AzureKeyCredential(AZURE_SEARCH_API_KEY)
else:
    # Use the current user identity to authenticate with Cognitive Search
    azure_search_credential = DefaultAzureCredential(exclude_shared_token_cache_credential = True)
...
if AZURE_OPENAI_API_KEY is not None:
    # Use the API keys to authenticate with Azure OpenAI
    openai.api_type = "azure"
    openai_token = None
    ...
    openai_token = azure_default_credential.get_token("https://cognitiveservices.azure.com/.default")
...
def chat():
    ...
    try:
        ...
        if not impl:
            return jsonify({"error": "unknown approach"}), 400
        r = impl.run(request.json["history"], request.json.get("overrides") or {}, request.json.get("filters") or {}, request.json["profile"])
        return jsonify(r)
    except Exception as e:
...
def ensure_openai_token():
    global openai_token
    if openai_token is not None and openai_token.expires_on < int(time.time()) - 60:
        openai_token = azure_default_credential.get_token("https://cognitiveservices.azure.com/.default")
        openai.api_key = openai_token.token
