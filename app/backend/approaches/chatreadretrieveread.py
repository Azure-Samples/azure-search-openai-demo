...
class ChatReadRetrieveReadApproach(Approach):
    ...
    """
    system_message_chat_conversation = """Assistant helps explain the content sourced from health insurance statements. Be brief in your answers.
...
{injected_prompt}
"""
    follow_up_questions_prompt_content = """Generate three very brief follow-up questions that the user would likely ask next about private health insurance statements. 
...
Only generate questions and do not generate any text before or after the questions, such as 'Next Questions'"""
    ...
    query_prompt_template = """Below is a history of the conversation so far, and a new question asked by the user that needs to be answered by searching through a knowledge base.
...
"""
    query_prompt_few_shots = [
        {'role' : USER, 'content' : 'What are my monthly waiting periods on my Corporate Benefit 80 cover?' },
        {'role' : ASSISTANT, 'content' : 'waiting periods months Corporate Benefit 80' },
        {'role' : USER, 'content' : 'Does my insurance include sinus treatments?' },
        {'role' : ASSISTANT, 'content' : 'include sinus treatments' }
    ]

    def __init__(self, search_client: SearchClient, chatgpt_deployment: str, chatgpt_model: str, embedding_deployment: str,
    ...
    def run(self, history: Sequence[dict[str, str]], overrides: dict[str, Any], filters: dict, profile: str) -> Any:
        ...
        if prompt_search_override is None:
            messages = self.get_messages_from_history(self.query_prompt_template,
                self.chatgpt_model,
                history,
                user_q,
    ...
    def get_messages_from_history(self, system_prompt: str, model_id: str, history: Sequence[dict[str, str]], user_conv: str, few_shots = [], max_tokens: int = 4096) -> list[Any] :
